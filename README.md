...
| [`CHANGELOG.md`](CHANGELOG.md) | 変更履歴（Keep a Changelog / SemVer。reusable workflow の版を記録） |
...
## Reusable workflows を他 repo から使う

`dependency-review` と `secrets-scan` は `on: workflow_call` を持つ **reusable workflow** です。他 repo から呼び出せます（**版で pin** すること。`@main` は中央の編集途中で全 consumer を壊すため非推奨）。

```yaml
# 他 repo の .github/workflows/security.yml
name: security
on:
  pull_request:
    branches: [main]
jobs:
  secrets:
    uses: thinkyou0714/.github/.github/workflows/secrets-scan.yml@v1.0.0
  deps: # dependency-review は public repo 専用（personal の private repo は GHAS 不可）
    uses: thinkyou0714/.github/.github/workflows/dependency-review.yml@v1.0.0
```

- 版の一覧は [Releases](https://github.com/thinkyou0714/.github/releases) / [`CHANGELOG.md`](CHANGELOG.md) を参照。より厳密に固定するなら tag ではなく commit SHA で pin する。
- `ci.yml` / `weekly-governance-audit.yml` / `stale-branch-gc.yml` は**このリポ専用**（`workflow_call` なし）。他 repo から `uses:` で参照しないこと。
